...
void CalcDepth(std::vector<int>& tree,
               std::vector<int>& depth,
               int node);

// Return true when two nodes(|node_a|, |node_b|) are located in the same sub tree in the |tree|.
// |depth| tree is used to choose which one is more close to root.
bool HasSameRoot(std::vector<int>& tree,
                 std::vector<int>& depth,
                 int node_a, int node_b);

// Get input file from argv[1], outputs into the stdout.
int main(int argc, char** argv) {
  std::vector<int> clean_vector(200000, -1);
  std::vector<int> tree(clean_vector);
  std::vector<int> depth(clean_vector);
...
bool HasSameRoot(std::vector<int>& tree,
  ...
  // Early out when two node has same depth.
